(function($){
	...
	with (jqUnit) {
		...
		describe('Claypool Models', 'Amazon SDB', {
           ...
           Artists.find({
               async:false,
               //query: _.items('*').where('$name').islike('Vox*'),
            ...
            Contacts.find({
                ...
                success: function(result){
                    ...
                    ok(result.$db$request, 'got db response metadata -> request id');
                    ok(result.$db$cpu, 'got db response metadata -> cpu usage');
                    equals(result.db$data[0].$firstName, 'Briana', 'got expected value');
                    equals(result.db$data[0].$lastName, 'Banks', 'got expected value');
                },
                error: function(result){
